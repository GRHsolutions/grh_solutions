...
const actualToken = localStorageUtil.get("usr_items_token"); // traer el token

const headers = {
  "Content-Type": "application/json;charset=utf-8", // especificar el content-type
  Authorization: actualToken != null ? `Bearer ${actualToken}` : null, // entregar el token a authorization
};

axios.interceptors.response.use( // controlar los codigos de respuesta, luego hay que hacer interceptores en ciertos casos
  (response) => {
      return response;
  },
  (error) => {
      if (error.response) {
          ...
          switch (status) {
              ...
              case 401:
                  ...
                  break;
              case 403:
                  console.error(`Error 403: Forbidden. Access denied. URL: ${url}`, data);
                  break;
              case 404:
